---
name: fs-init
description: Initialize frontend-system by inspecting an existing project or building a new frontend from an approved revision, preserving optional design-provider settings. Use when the user asks to initialize, set up, or start frontend-system.
---
...
unapproved, use `fs-revise` for domain-first discussion and explicit approval before
...
features follow through `fs-implement`.
...
5. Continue with `fs-inspect` even when external setup is blocked. Finish with separate inspection and OpenDesign readiness results, any user action still needed, and a concrete `fs-revise` or `fs-implement` example for this project. Full OpenDesign setup is incomplete until connection, authentication, and project binding have been verified; inspection requires `.frontend-system/init.md` and accounted-for area evidence. Legacy `project.md` remains preserved.
