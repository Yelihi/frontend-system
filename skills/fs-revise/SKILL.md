---
name: fs-revise
description: Discuss domain rules and architectural tradeoffs and write or revise a project target design. Use when the user wants a revision, architecture direction, or a domain-first plan for an existing or new frontend project. Does not execute a refactoring.
---
...
1. Call `get_work_context` with mode `prepare`. For existing projects, establish or
   refresh missing/stale inspection through `fs-inspect`. For new projects, inventory
