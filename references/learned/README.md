...
현재 인덱스에는 접근성·React·코드 품질·번들링·디버깅·HTML·CSS·JavaScript·웹 플랫폼·Web API의 개념 참조가 있습니다. 원본 등록만으로 배포된 것으로 간주하지 않으며 sync를 통해 참조와 인덱스를 함께 작성해야 합니다. 원본 자료는 `knowledge/source/`에 보존합니다.

TypeScript 모듈 설정, Next.js 구성 경계, TanStack Query, INP·레이아웃 진단, PWA UX, 서버 인가, 마이크로 프런트엔드와 Chromium·WebGL 심화 자료도 주제별로 선별했습니다. 각 참조의 앞부분에서 **참고 상황 → 판단에 사용할 내용 → 적용하지 않는 경우**를 확인합니다. 이 목적은 검색 요약에도 반영하며, 실제 제품에서 채택한 이유와 검증 결과는 해당 프로젝트에 별도로 기록합니다.

MDN의 넓은 허브와 가이드는 확인한 개요 범위의 참조입니다. 해당 사이트 전체나 모든 하위 API를 완독한 자료가 아니며, 기능 도입 시 참조의 적용 범위와 세부 원문을 확인합니다. 폐기 API는 이관 검토용으로 구분합니다. 원본 저장소의 검토 기록과 imported 요약에 처리 범위와 검증 한계를 남깁니다.
